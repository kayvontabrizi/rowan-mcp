...
def submit_irc_workflow(
    ...
    method: Annotated[
        str,
        "Computational method for IRC (e.g., 'uma_m_omol', 'gfn2_xtb', 'r2scan_3c')",
    ] = "uma_m_omol",
    preopt: Annotated[
        bool, "Whether to pre-optimize the transition state before IRC step"
    ] = True,
    step_size: Annotated[
        float, "Step size for IRC path tracing in Bohr (typically 0.03-0.1)"
    ] = 0.05,
    max_irc_steps: Annotated[
        int, "Maximum number of IRC steps in each direction from TS"
    ] = 30,
    name: Annotated[
