...
def submit_fukui_workflow(
    initial_molecule: Annotated[
        str, "SMILES string of the molecule to calculate Fukui indices for"
    ],
    optimization_method: Annotated[
        str, "Method for geometry optimization (e.g., 'gfn2_xtb', 'uma_m_omol')"
    ] = "gfn2_xtb",
    fukui_method: Annotated[
        str, "Method for Fukui indices calculation (e.g., 'gfn1_xtb', 'gfn2_xtb')"
    ] = "gfn1_xtb",
    solvent_settings: Annotated[
        str, "JSON string for solvent settings. Empty string for vacuum"
    ] = "",
    name: Annotated[
    ...
    try:
        ...
        fukui_settings = stjames.Settings(
            method=fukui_method, solvent_settings=parsed_solvent_settings
        )
        ...
        if (
            "scf_settings" in opt_settings_dict
            and "soscf" in opt_settings_dict["scf_settings"]
        ):
        ...
        if (
            "scf_settings" in fukui_settings_dict
            and "soscf" in fukui_settings_dict["scf_settings"]
        ):
