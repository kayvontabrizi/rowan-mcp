...
def submit_scan_workflow(
    ...
    scan_settings: Annotated[
        str,
        'JSON string of scan parameters: \'{"type": "dihedral"/"bond"/"angle", "atoms": [1-indexed], "start": value, "stop": value, "num": points}\'',
    ] = "",
    calculation_engine: Annotated[
        str, "Computational engine: 'omol25', 'xtb', 'psi4'"
    ] = "omol25",
    calculation_method: Annotated[
        str, "Computational method (e.g., 'uma_m_omol', 'gfn2-xtb', 'b3lyp-d3bj')"
    ] = "uma_m_omol",
    wavefront_propagation: Annotated[
        bool, "Whether to use wavefront propagation for scan"
    ] = True,
    name: Annotated[
    ...
    if parsed_scan_settings is not None:
        ...
        missing_fields = [
            field for field in required_fields if field not in parsed_scan_settings
        ]
        if missing_fields:
            raise ValueError(
                f"Missing required fields in scan_settings: {missing_fields}"
            )
