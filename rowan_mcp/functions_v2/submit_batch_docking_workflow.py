...
def submit_batch_docking_workflow(
    smiles_list: Annotated[
        str,
        'JSON string list of ligand SMILES to dock (e.g., \'["CCO", "CC(=O)O", "c1ccccc1"]\')',
    ],
    protein: Annotated[str, "Protein UUID or PDB content/path for docking target"],
    pocket: Annotated[
        str,
        "JSON string defining binding pocket coordinates or 'auto' for automatic detection",
    ],
    ...
    exhaustiveness: Annotated[
        int, "Search exhaustiveness parameter (higher = more thorough, slower)"
    ] = 8,
    name: Annotated[
    ...
    try:
        ...
        if "," in smiles_list:
            parsed_smiles_list = [
                s.strip() for s in smiles_list.split(",") if s.strip()
            ]
        else:
    ...
    # Submit the workflow
    logger.info(
        f"Submitting batch docking workflow for {len(parsed_smiles_list)} ligands"
    )
