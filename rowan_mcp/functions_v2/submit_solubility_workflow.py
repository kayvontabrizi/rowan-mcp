...
def submit_solubility_workflow(
    initial_smiles: Annotated[
        str, "SMILES string of the molecule for solubility prediction"
    ],
    solvents: Annotated[
        str,
        'JSON string list of solvents as SMILES or names (e.g., \'["water", "ethanol", "CCO"]\'). Empty string uses defaults',
    ] = "",
    temperatures: Annotated[
        str,
        "JSON string list of temperatures in Kelvin (e.g., '[298.15, 310.15]'). Empty string uses default range",
    ] = "",
    name: Annotated[
    ...
    if solvents:
        ...
        try:
            ...
            for i, solvent_smiles in enumerate(parsed_solvents):
                ...
                if mol is None:
                    raise ValueError(
                        f"Invalid solvent SMILES: '{solvent_smiles}' at position {i}"
                    )
        except ImportError:
    ...
    if temperatures:
        ...
        if temperatures.startswith("[") and temperatures.endswith("]"):
            ...
            try:
                ...
                # Failed to parse as JSON, try as comma-separated
                temperatures = (
                    temperatures.strip("[]").replace('"', "").replace("'", "")
                )
                parsed_temperatures = [
                    float(t.strip()) for t in temperatures.split(",") if t.strip()
                ]
