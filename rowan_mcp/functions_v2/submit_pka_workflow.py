...
def submit_pka_workflow(
    ...
    pka_range: Annotated[
        List[float], "pKa range [min, max] to search (e.g., [2, 12])"
    ] = [2, 12],
    deprotonate_elements: Annotated[
        str,
        "Comma-separated elements for deprotonation (e.g., 'N,O,S'). Empty for auto-detect",
    ] = "",
    protonate_elements: Annotated[
        str,
        "Comma-separated elements for protonation (e.g., 'N,O'). Empty for auto-detect",
    ] = "",
    name: Annotated[
    ...
    pka_range_tuple = (
        tuple(pka_range)
        if len(pka_range) == 2
        else (pka_range[0], pka_range[1] if len(pka_range) > 1 else pka_range[0])
    )
