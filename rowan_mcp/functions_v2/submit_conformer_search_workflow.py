...
def submit_conformer_search_workflow(
    initial_molecule: Annotated[
        str, "SMILES string representing the initial structure"
    ],
    final_method: Annotated[
        str,
        "Final optimization method (e.g., 'aimnet2_wb97md3', 'r2scan_3c', 'wb97x-d3_def2-tzvp')",
    ] = "aimnet2_wb97md3",
    solvent: Annotated[
        str, "Solvent for implicit solvation (SMILES or name). Empty string for vacuum"
    ] = "",
    transition_state: Annotated[
        bool, "Whether to search for transition state conformers"
    ] = False,
    name: Annotated[
    ...
    try:
        ...
        opt_settings = stjames.Settings(
            ...
            solvent_settings=(
                {"solvent": solvent, "model": solvent_model} if solvent else None
            ),
        ...
        if "optimization_settings" in msos_dict:
            for opt_setting in msos_dict["optimization_settings"]:
                if (
                    "scf_settings" in opt_setting
                    and "soscf" in opt_setting["scf_settings"]
                ):
